...
[
  {
    ...
    "end": 2.5,
    "text": "原 SRT 字幕文本",
    "validateElementDesign": "P1-001 含 3 个元素：标题'核心竞争力' 60px 居中、副标题'AI 时代' 36px 居中偏上、配图 480x320 居中偏下。元素垂直堆叠无重叠，文字与背景对比度足够，整体合理。"
  }
]
```
...
- **目的**：让 AI 在设计画面时强制自我审视"字幕和画面是否匹配"，避免画完再说
- **长度**：30-200 字
- **必含 element id**：至少 1 个 `P{n}-{nnn}` 格式（如 `P1-002`），强制 AI 关联到具体元素
- **必含内容**：
  1. 该字幕区间内有哪些元素（id 列表 + 视觉功能）
  2. 元素布局是否合理（位置/尺寸/层级/对比度）
  3. 整体评价（合理 / 不合理 + 原因）
- **反例（会被 fail-fast）**：
  - ❌ `"合理"` —— 太短，无 element id
  - ❌ `"元素布局合理，3 个元素组成"` —— 无 element id
  - ❌ `""` —— 空
- **正例**：见上方示例

> 校验在 `selfcheck.js`（Skill 端）和 `projectValidator.js`（Server 端）双端执行，缺失/过短/无 element id 都会被拒。
