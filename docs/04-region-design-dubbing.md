...
| 组件类型 | end 规则 |
|---------|---------|
| 锚点（关键信息）| end ≥ start + 2秒（关键信息停留 ≥ 2秒）|
| 核心内容 | end ≥ start + 1.5秒 |
| 辅助内容 | end ≥ start + 1秒 |
| 装饰元素 | end ≥ start + 0.5秒 |
| 末组件 | end ≤ 区域end - 0.5秒（留出淡出时间）|
...
**时间分配示例**（区域 5-10秒，标准档位）：

| 组件 | start | end | 说明 |
|------|-------|-----|------|
| TitleComponent（锚点）| 5.0 | 9.5 | 关键信息，停留4.5秒 |
| TextComponent | 5.5 | 8.5 | 辅助内容 |
| ImageComponent | 6.0 | 9.0 | 核心内容 |
| BadgeComponent | 7.0 | 8.0 | 装饰元素 |
...
| `customStyle` | 步骤 7（API 字段） | `{ "level1": { ... } }` |
| `start` | 时间轴 | 5.0 |
| `end` | 时间轴 | 10.0 |
...
- 还有区域？→ 返回 [步骤4：区域设计与生成JSON](04-region-design-dubbing.md)
