...
- [ ] `design-P{n}.md` 已存在
- [ ] `design-P{n}.md` 内容完整（包含组件布局、出场顺序、配色）

**严禁**：没有 `design-P{n}.md` 就直接写 `regions/P{n}.json`
...
> 注意：`source_design_doc` 字段必须记录区域设计文档来源。
...
```bash
node scripts/validate-region.js {workdir}/{skillProjectId}/skeleton.json {workdir}/{skillProjectId}/regions/{regionName}.json
```
...
```bash
node scripts/validate.js {workdir}/{skillProjectId}/regions/{regionName}.json
```
...
- [E] regionName 与文件名一致
- [E] source_design_doc 字段存在且不为空（记录区域设计文档来源）
