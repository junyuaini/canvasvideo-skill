...
| 引用规则 | `rules/06-components.md`、`rules/03-layout.md` |
...
约束：
- `y` 必须在区域内严格递增（详见 `rules/03-layout.md` §R2）
- `w` ≤ `viewport.width - 40`
...
- [ ] regionId 与文件名一致
- [ ] 组件总数 ≤ 5 个
- [ ] 每个组件有完整的字段（id, type, content, position, customStyle, start, end）
- [ ] `id` 格式正确（如 P1-001）
