...
> **两个示例覆盖两种模式**，按用户场景选对应的示例照着改即可。

---

## 分合示例-创作/（创作模式 · 演示"骨架 → 区域 → 合并"工作流）

- **场景**：用户输入主题/文案，AI 自动生成画面、BGM，无需配音
- **特点**：
  - 没有配音音频，配 `assets/placeholders/bgm/*.mp3`
  - 没有字幕（`subtitles` 为空）
  - 区域之间画面靠节奏推进
- **包含**：
...
- **包含**：同创作模式（`skeleton.json` / `regions/P{n}.json` / `merged.json`）
...
2. **assets 路径必须改**：
   - 创作模式：`assets/placeholders/bgm/*.mp3` → 实际 BGM 路径
   - 口播模式：`assets/placeholders/voiceover/*.mp3` → 实际配音路径
