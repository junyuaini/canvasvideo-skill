...
- 时长：{N} 秒（**口播模式 = 音频时长 = SRT 最晚 end 时间，严禁自行调整**）
- 模式：{创作 / 口播}
- 主题：{white / black}（默认 black 沉浸黑）
- 总区域数：{N}（按 timing-rules.md 节奏门槛推算；**口播模式按 SRT 段落结构划分**）
- 视频比例：4:3（780×585）
- BGM：{自动配 / 用户拒绝 / 口播模式不用}

> **口播模式 [硬规则] 一切以 SRT 为准**（详见 [`../../references/mode-rules.md`](../../references/mode-rules.md) §5）：
> - 字幕文本 100% 不动，严禁改写、压缩、扩展
> - 总时长 = 音频时长 = SRT 最晚 end 时间
> - 每个区域对应一段连续字幕，组件内容必须与当前字幕语义一致
> - 节奏门槛与 SRT 冲突时，优先保 SRT 对齐，接受 warning
