...
**转换流程**（必须使用脚本，严禁 LLM 自行解析）：
1. **调用脚本解析**：`const { parseSrt } = require('./scripts/srt-parser'); const subtitles = parseSrt(srtPath);`
2. **原样使用**：`subtitles` 数组直接写入 project.json，**文本和时间戳都不动**

> 脚本位置：`scripts/srt-parser.js`
> 功能：自动处理时间戳转换（`00:00:02,366` → `2.366`）、多行文本、空行等
...
**严禁**：
- ❌ **LLM 自己解析 SRT 文件**（必须使用 `scripts/srt-parser.js`）
- ❌ **LLM 自己写 subtitles 数组**（必须调用 parseSrt 脚本）
- ❌ **改动 SRT 文本内容**（哪怕是一个字、一个标点）
- ❌ **调整时间戳**（start/end 必须和 SRT 完全一致）
- ❌ **合并或拆分 SRT 条目**（SRT 有几条，subtitles 就有几条，一一对应）
- ❌ **漏掉 SRT 中的任何一条字幕**

**时间戳转换**（由脚本自动处理，LLM 不需要手动计算）：
- 脚本自动将 `00:01:23,456` → `83.456` 秒
- 保留 3 位小数精度
