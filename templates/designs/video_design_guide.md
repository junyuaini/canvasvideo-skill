...
| 区域 | 时长 | 末组件→end | 节奏描述 |
|------|---|---|---------|
| P1 | 8s | 2s | 标题先出 → 金句 1.5s 后出现 → 收尾图片 6s 出现 |
| P2 | 10s | 2s | 标题先出 → 3 个痛点 Badge 依次出现（间隔 1.5s） → 方案 Badge 在 16s 出现做收尾 |
| P3 | 12s | 2s | 标题 + 副标 → 4 个步骤 Card 依次出现（每个间隔 1.5-4s） → 最后步骤在 28s 出现 |
| P4 | 12s | 2s | 标题先出 → 4 个数据 Shock 依次出现 → 最后数据在 40s 收尾 |
| P5 | 10s | 2s | 标题 → 金句 → 按钮 Badge → 图标光线收尾 50s |
| P6 | 8s | 2s | 行动召唤大字 → 二维码 58s 收尾 |
...
| P2 |  | 痛点2 | 11 | 18 | — |
| P2 |  | 痛点3 | 12.5 | 18 | — |
| P2 |  | **方案 Badge（收尾）** | 16 | 18 | **2.0s ✅** |
| P3 | 12s | 标题 | 18 | 30 | — |
| P3 |  | 副标 | 19 | 30 | — |
| P3 |  | 步骤1 | 20.5 | 30 | — |
| P3 |  | 步骤2 | 22 | 30 | — |
| P3 |  | 步骤3 | 24 | 30 | — |
| P3 |  | **步骤4（收尾）** | 28 | 30 | **2.0s ✅** |
| P4 | 12s | 标题 | 30 | 42 | — |
| P4 |  | 数据1 | 31.5 | 42 | — |
| P4 |  | 数据2 | 33 | 42 | — |
| P4 |  | 数据3 | 35 | 42 | — |
| P4 |  | **数据4（收尾）** | 40 | 42 | **2.0s ✅** |
| P5 | 10s | 标题 | 42 | 52 | — |
| P5 |  | 金句 | 43.5 | 52 | — |
| P5 |  | 按钮 Badge | 45 | 52 | — |
| P5 |  | **图标/光线（收尾）** | 50 | 52 | **2.0s ✅** |
| P6 | 8s | 行动召唤 | 52 | 60 | — |
| P6 |  | **二维码（收尾）** | 58 | 60 | **2.0s ✅** |

> **关键观察**：
> 1. 每个区域都有"收尾组件"在 end-2s 出现，避免最后画面静止
> 2. 同区域内相邻组件 start 间隔最大 4s（在 P3/P4 中），**实际写设计时建议控制在 ≤ 3s**
> 3. 单区域时长 ≤ 12s，远低于 15s 上限
...
| **末组件停留 ≤ 2s** | ✅/❌ | 列出每个区域末组件→end 间隔（例：P1=2.0, P2=1.8, ...） |
| **创作模式单区域 ≤ 15s** | ✅/❌ | 列出最长区域 + 时长（例：P3=12s ✅） |
| **组件尺寸与内容匹配（无溢出）** | ✅/❌ | 每个区域：文字组件 h ≥ 行数×行高+边距；最后一个组件 y+h ≤ viewport.height-10 |
| **组件 Y 坐标连续递增（无重叠）** | ✅/❌ | 每个区域按 y 排序，检查 sorted[i].y ≥ sorted[i-1].y + sorted[i-1].h + 10 |
...
| 深色背景→亮色文字 | ✅ 通过 | 文字色 #FFFFFF |
| 文字叠图→白色 + shadow / backdropFilter | ✅ 通过 | 所有图片上方文字已加 textShadow 或 backdropFilter |
| 彩色背景块→文字色与背景对比鲜明 | ✅ 通过 | 橙色背景用黑字，蓝色背景用白字 |
| 无低对比度配色（如浅灰底+更浅灰字） | ✅ 通过 | 所有文字与背景对比度可辨识 |
...
| **门槛 7.1 末组件→区域 end ≤ 2s** | 所有区域满足 | ✅/❌ | 各区域间隔 [P1=1.5, P2=1.8, ...]；超标区域：[] |
| **门槛 7.2 相邻组件 start 间隔 ≤ 3s** | 所有区域满足 | ✅/❌ | 最大间隔 X 秒（在 PX 区） |
| **门槛 7.3 创作模式单区域 ≤ 15s** | 创作模式所有区域满足 | ✅/❌ | 最长区域 X 秒（在 PX）；口播模式豁免 |
| **门槛 7.4 创作模式总密度 ≥ 0.6/s** | 创作模式总密度满足 | ✅/❌ | 组件 X 个 / 时长 Ys = Z；口播模式豁免 |
