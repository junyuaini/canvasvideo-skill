...
> 3. **同区域内相邻组件 start 间隔**：口播模式 ≤ 3s（推荐 0.8-2.5s）；创作模式 ≤ 1s（推荐 0.3-1.0s）；详见 timing-rules.md 门槛 2
> 4. ⚠️ 上表 P3/P4/P5/P6 末段（如 P5 按钮→收尾 6.5s、P6 行动召唤→二维码 7s）**仅作"末组件停留"演示，违反门槛 2**。实际写设计时**必须在末段也补 1-2 个过渡组件**（比如 P5 在 45s-51.5s 之间加副文案 / 装饰元素），让相邻间隔都落到上限内
> 5. 单区域时长 ≤ 12s，远低于 15s 上限
