...
>
> **🚨 口播模式 [硬规则] 一切以 SRT 为准**（详见 [`../../references/mode-rules.md`](../../references/mode-rules.md) §5）：
> - **字幕文本 100% 不动**：严禁改写、压缩、扩展、润色
> - **总时长以音频/SRT 为准**：视频总时长 = 音频时长 = SRT 最晚 end 时间，严禁自行调整
> - **组件出现/消失必须匹配字幕当前意思**：每个区域对应一段连续字幕，组件内容必须与当前字幕语义一致
> - 节奏门槛（区域时长 ≤ 8s、组件间隔 ≤ 3s 等）与 SRT 冲突时，**优先保 SRT 对齐，接受 warning**
>
> **🚨 口播模式素材白名单（硬规则）**：
> - **只允许使用两种素材**：音频文件（`.mp3 / .wav / .m4a`）和字幕文件（`.srt`）
> - **严禁将其他格式文件作为创作依据**：素材目录下的 `.txt / .doc / .png / .jpg / .pdf / .mp4` 等文件，**一律不得用于口播视频的内容提取、参考或创作**
> - **例外**：用户明确说"也用这张图片/这个文档"时，才可将该文件纳入素材清单
...
> ⚠️ **区域时长强约束（按模式区分；详见 [`../../references/timing-rules.md`](../../references/timing-rules.md) 门槛 3、门槛 4）**：
> - 单一区域 duration：**创作模式 ≤ 5 秒**；**口播模式 ≤ 8 秒**（**严禁 25s/30s 这种"省事的整数"**）
> - 区域数推荐：
>   - 创作模式：30s ≥ 6 个；60s ≥ 12 个；120s ≥ 24 个；180s ≥ 36 个
>   - 口播模式：30s ≥ 4 个；60s ≥ 8 个；120s ≥ 15 个；180s ≥ 23 个
> - 各区域时长可不等长（按内容多少灵活分配：创作 3-5s 之间；口播 4-8s 之间）
...
**产出示例**（60 秒视频，**口播模式 8 个区域**，时长不等长，全部 ≤ 8s）：

> ⚠️ 本示例为**口播模式**（区域上限 8s，60s 推荐 ≥ 8 个区域）。
> **创作模式** 60s 视频应拆为 ≥ 12 个区域（单区域 ≤ 5s），不能直接抄本示例。

| 区域 | 时间段 | 时长 | 内容类型 | 设计意图（1句话） | 建议组件数 | 焦点组件 | 建议图片素材 |
|------|--------|---|---------|----------------|-----------|---------|------------|
| P1 | 0-7   | 7s | HOOK    | 让用户3秒内记住产品名字和核心价值 | 4 | ShockComponent | 产品logo图 |
| P2 | 7-15  | 8s | PAIN-1  | 让用户感受到传统方式的痛点（场景） | 5 | Badge（红色） | 痛点场景图 |
| P3 | 15-22 | 7s | PAIN-2  | 用对比放大痛点（数据） | 5 | Shock（数字） | 对比图 |
| P4 | 22-30 | 8s | SOLVE-1 | 展示方案前 2 个步骤 | 6 | 流程图（步骤） | 步骤截图 |
| P5 | 30-38 | 8s | SOLVE-2 | 展示方案后 2 个步骤 | 6 | 流程图（步骤） | 步骤截图 |
| P6 | 38-46 | 8s | RESULT  | 用数据证明产品能力 | 6 | Shock（大数字） | 成果截图 |
| P7 | 46-54 | 8s | CTA-1   | 引导用户立即行动 | 5 | Badge（CTA按钮） | 产品截图 |
| P8 | 54-60 | 6s | CTA-2   | 收束情绪 + 二维码 | 4 | ShockComponent | 二维码 |

> **总组件数 ≈ 41**，时长 60s → 密度 0.68（满足创作模式门槛 4 的 ≥ 0.6；口播模式无此门槛，但密度足够才能避免单调）
...
> 3. **区域时长（按模式区分）**：创作模式 ≤ 5 秒；口播模式 ≤ 8 秒。超出必须拆分
...
> ⚠️ **节奏强约束（按模式区分；详见 [`../../references/timing-rules.md`](../../references/timing-rules.md)）**：
> - 单一区域 duration：创作模式 ≤ 5 秒；口播模式 ≤ 8 秒
> - 末组件 start 到区域 end 间隔：创作模式 ≤ 1 秒；口播模式 ≤ 2 秒
> - 同区域相邻组件 start 间隔：创作模式 ≤ 1 秒；口播模式 ≤ 3 秒
> - 视频总时长 60s 推荐区域数：创作模式 ≥ 12 个；口播模式 ≥ 8 个

**产出示例**（60 秒视频，**口播模式 8 区域，全部节奏门槛通过**）：

| 区域 | 时长 | 元素 | start | end | 相邻间隔 / 末→end |
|------|---|------|-------|-----|---|
| P1 | 7s | 标题 | 0 | 7 | — |
| P1 |  | 副标 | 1.5 | 7 | 1.5s ✅ |
| P1 |  | 金句 | 3.2 | 7 | 1.7s ✅ |
| P1 |  | **图片（收尾）** | 5.8 | 7 | 2.6s 间隔；**末→end 1.2s ✅** |
| P2 | 8s | 标题 | 7 | 15 | — |
| P2 |  | 痛点1 | 8.5 | 15 | 1.5s ✅ |
| P2 |  | 痛点2 | 10 | 15 | 1.5s ✅ |
| P2 |  | 痛点3 | 11.5 | 15 | 1.5s ✅ |
| P2 |  | **方案 Badge（收尾）** | 13.5 | 15 | 2.0s 间隔；**末→end 1.5s ✅** |
| P3 | 7s | 标题 | 15 | 22 | — |
| P3 |  | 数据1 | 16.5 | 22 | 1.5s ✅ |
| P3 |  | 数据2 | 18 | 22 | 1.5s ✅ |
| P3 |  | **对比图（收尾）** | 20.5 | 22 | 2.5s 间隔；**末→end 1.5s ✅** |
| P4 | 8s | 标题 | 22 | 30 | — |
| P4 |  | 步骤1 | 23.5 | 30 | 1.5s ✅ |
| P4 |  | 步骤2 | 25 | 30 | 1.5s ✅ |
| P4 |  | **截图（收尾）** | 28.5 | 30 | 2.5s 间隔；**末→end 1.5s ✅** |
| P5 | 8s | 标题 | 30 | 38 | — |
| P5 |  | 步骤3 | 31.5 | 38 | 1.5s ✅ |
| P5 |  | 步骤4 | 33 | 38 | 1.5s ✅ |
| P5 |  | **截图（收尾）** | 36.5 | 38 | 2.5s 间隔；**末→end 1.5s ✅** |
| P6 | 8s | 标题 | 38 | 46 | — |
| P6 |  | 数据1 | 39.5 | 46 | 1.5s ✅ |
| P6 |  | 数据2 | 41 | 46 | 1.5s ✅ |
| P6 |  | **数据3（收尾）** | 44.5 | 46 | 2.5s 间隔；**末→end 1.5s ✅** |
| P7 | 8s | 标题 | 46 | 54 | — |
| P7 |  | 金句 | 47.5 | 54 | 1.5s ✅ |
| P7 |  | **按钮 Badge（收尾）** | 52.5 | 54 | 2.5s 间隔；**末→end 1.5s ✅** |
| P8 | 6s | 行动召唤 | 54 | 60 | — |
| P8 |  | 副文案 | 55.5 | 60 | 1.5s ✅ |
| P8 |  | **二维码（收尾）** | 58.8 | 60 | 1.7s 间隔；**末→end 1.2s ✅** |

> **关键观察**：
> 1. 全表"末组件 start → 区域 end"间隔分布：1.2s / 1.5s / 1.5s / 1.5s / 1.5s / 1.5s / 1.5s / 1.2s（中位数 1.5s，全部 ≤ 2s）—— 符合口播模式门槛 1
> 2. 全表"相邻组件 start 间隔"最大 2.6s，全部 ≤ 3s —— 符合口播模式门槛 2
> 3. 全表区域 duration ≤ 8s —— 符合口播模式门槛 3
> 4. **创作模式示例**应进一步收紧：区域 ≤ 5s、末→end ≤ 1s、相邻 ≤ 1s（详见 [`../../references/timing-rules.md`](../../references/timing-rules.md)）
> 5. **本表是"合格示例"**，可直接作为口播 60s 模板套用
...
```
| 末组件停留 ≤ 2s | ✅ | P1=1.2, P2=1.5, P3=1.5, P4=1.5, P5=1.5, P6=1.5, P7=1.5, P8=1.2（中位数 1.5s） |
```
...
- **口播模式**：
  - 音频/SRT **必须**标 `[已具备]`（不允许 AI 生成）
  - **素材白名单**：只允许音频（`.mp3/.wav/.m4a`）和字幕（`.srt`）作为用户素材，其他格式（`.txt/.doc/.png/.jpg/.pdf/.mp4` 等）**不得列入素材清单**，除非用户明确说"也用这张图片/文档"
