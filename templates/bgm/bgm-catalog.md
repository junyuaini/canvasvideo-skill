...
> ⚠️ **当前仓库中尚未实际放入 mp3 文件**（避免仓库体积膨胀 + 版权追溯）。
> 部署 Skill 时由管理员一次性手动入库，或在用户首次使用时按需提示用户下载。
...
**推荐工作流**：
1. 在 Pixabay Music 各搜索 `tech / warm coffee / uplifting / corporate / pop / cinematic`
2. 每个风格选 1 首 30-60 秒的曲目下载（注意压缩到 96-128 kbps，控制每首 ≤ 1MB）
3. 重命名为本表中的标准文件名
4. 放到 `canvasvideo-skill/templates/bgm/` 下
