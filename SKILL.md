...
[第零次] 安装就位（用户无感）
    ↓
[第一次] 理解需求 + 设计路径选择（协作设计 / 自主设计）+ 模式选择（创作 / 口播）
    ↓
    ├─ [协作设计] [第二次] 生成完整 design.md → [第三次] 用户确认 → [第四次] 打包上传
    │              （子流程 → video_design_guide.md，含 11 个步骤）
    │
    └─ [自主设计] [第二次] 生成完整 design.md → LLM 自我确认 → [第四次] 打包上传
...
- 如果用户素材目录下有多个文件，**只以 .mp3/.wav/.m4a 和 .srt 作为口播素材**，其他格式（如 .txt/.doc/.png 等）一律忽略，除非用户明确说"也用这张图片"。
