...
[第零次] 安装就位（用户无感）
    ↓
[第一次] 理解需求 + 设计路径选择（协作设计 / 自助设计）+ 模式选择（创作 / 口播）
    ↓
    ├─ [协作设计] [第二次] 生成完整 design.md → [第三次] 多轮微调 → 用户确认 → [第四次] 打包上传
    │              （子流程 → video_design_guide.md，含 11 个步骤）
    │
    └─ [自助设计] [第二次] 生成精简 design-quick.md（**不要求用户确认**）→ 直接 [第四次] 打包上传
...
[第六次] 视频迭代（局部编辑：直接改 project.json，不重走 design.md）
    ↓
   ⚠️ 已经走"自助设计"的项目不支持"补做完整 design.md"。想要完整设计 = 重新走协作设计流程从头来。
...
**子流程**：
- [`templates/designs/video_design_guide.md`](./templates/designs/video_design_guide.md) —— design.md 步骤 0~11 子流程（被第二次交互**协作设计**调用）
- [`templates/designs/design-quick.md`](./templates/designs/design-quick.md) —— 精简版设计文档模板（被第二次交互**自助设计**调用，50 行内一次性产出）
...
- 用户选 B 自助设计后：跳过第三次交互（用户确认环节），第二次直接生成 design-quick.md 后**自动进入第四次**
...
```
我帮你做这个视频，简单确认下：
1. 设计路径：A 协作设计（生成完整 design.md + 你确认） / B 自助设计（直接生成）
2. 时长：5 / 15 / 30 / 60 秒
3. 目标受众：大众 / 专业人士 / 年轻人？
...
**自助设计（路径 B）创作模式必问 1 项**（受众默认大众）：

| 字段 | 询问理由 | 默认值兜底（用户拒答时） |
|------|---------|-----------------------|
| **视频时长** | 决定区域数、节奏密度 | **15 秒**（自助设计专属默认值） |

**口播模式必问 2 项**（不论协作设计还是自助设计，**绝不问时长和受众**）：
...
- **路径选择确定后必须把 `path` 字段写到 `project-state.json`**：`standard` = 协作设计，`quick` = 自助设计。后续脚本会读取该字段判断流程分支

### 严禁
- ❌ 用户只说了"做一个 XX 视频"就直接进入第二次交互生成 design.md
- ❌ 强行追问主题 / 受众（创作模式才问受众；主题永远不问；自助设计连受众都不问；**口播模式绝不问时长和受众**）
...
**[自助设计]**：[`templates/designs/design-quick.md`](./templates/designs/design-quick.md) —— 精简版（50 行内），单文件，5 块结构（基础设定 / 区域划分 / 主视觉决策 / 节奏曲线 / 自检清单）。**一次性输出**即可，**不要求用户确认**，生成后**自动跳过第三次交互**直接进入第四次。
...
### 必查规则
- **协作设计**：所有 references/ 规则（子流程会逐步引用）
- **自助设计**：references/components-catalog.md（选型决策）+ references/timing-rules.md（节奏门槛）+ references/layout-rules.md（布局公式），其它规则在写 project.json 时按需查

### 严禁
- ❌ 跳过子流程任何步骤
- ❌ 一次性输出所有产出（协作设计必须按步骤逐步生成；自助设计才允许一次性输出）
- ❌ 留空表格单元格 / 自检全部填"通过"
- ❌ 使用 `theme` 之外的全局主题机制（必须用 customStyle）
- ❌ 组件 ID 用英文单词（必须用 `{区域}-###` 格式）
- ❌ **自助设计跳过 design-quick.md 直接写 project.json**（精简版仍是必要的思考脚手架）

---

## 四、第三次交互：多轮设计微调（直至确认）

> ⚠️ **自助设计跳过本节**——design-quick.md 生成后自动进入第四次，不等用户确认。
> 本节只对协作设计生效。
...
- **自助设计下** LLM 在生成 design-quick.md 后**立即调用** `state.markDesignConfirmed()` 自我标记为已确认，无需用户介入（这是自助设计的核心简化）
