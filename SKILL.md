...
#### 2.6.3 占位图策略：Picsum 随机图 + AggregateComponent 叠水印

**默认主用方案**（稳定、快速、真实摄影图）：
- 图源：**Picsum Photos API** `https://picsum.photos/seed/{seed}/{width}/{height}` —— 免费、免 key、长期稳定
- 水印：用 **AggregateComponent 把 ImageComponent 和 ShockComponent 组合在一起**，ShockComponent 显示中文胶囊水印"※ 演示图片 请替换"
- 优势：真实摄影图质量高；通过 `seed` 固定图片，同一主题每次返回相同图；图床字体不支持中文也无所谓（我们自己叠中文水印）

**Seed 命名要点**：
- 用英文主题词，简洁有意义，如 `couple-love`、`tech-ai`、`corporate-team`
- 同一视频内不同区域至少加序号区分，如 `couple-love-1`、`couple-love-2`
- 严禁写中文 seed、写太长的 seed（> 30 字符）、同一视频内不同区域用完全相同的 seed

**离线兜底方案**：
- Picsum 不可达 / 超时 / 用户拒绝远程图 → 用本地 SVG（`./assets/placeholders/{light|dark}/{hint}.svg`）
- SVG 自带水印，**不要再叠 Aggregate**，否则会双水印

**详见 [`templates/placeholders/url-factory.md`](./templates/placeholders/url-factory.md)**：
- 10 个常见主题 × 推荐 seed 速查表
- 极简白 / 沉浸黑 主题适配的 Aggregate 完整模板（含胶囊样式）
- 决策表 + 严禁清单

**严禁**：
- 在 ImageComponent 直接写 Picsum URL 而不套 Aggregate（图片裸奔没水印，用户分不清是占位还是真实素材）
