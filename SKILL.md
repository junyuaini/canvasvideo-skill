...
## 总体流程（7 次交互 + Plan and Execute）

```
[第零次] 安装就位（用户无感）
    ↓
[第一次] Plan 阶段：理解需求 + 路径选择 + 模式选择 + 制定待办清单
    ↓
    用户确认待办清单
    ↓
[第二次] Execute 阶段开始：生成完整 design.md
    ↓
    ├─ [协作设计] [第三次] 用户确认 design.md → [第四次] 打包上传
    │              （子流程 → video_design_guide.md，含 11 个步骤）
    │
    └─ [自主设计] LLM 自我确认 design.md → [第四次] 打包上传
                       （子流程 → video_design_guide.md，含 11 个步骤，**跳过用户确认**）
    ↓
[第五次] 用户在浏览器查看视频
    ↓
[第六次] 视频迭代（局部编辑：直接改 project.json，不重走 design.md）
    ↓
   ⚠️ 已经走"自主设计"的项目不支持"补做完整 design.md"。想要完整设计 = 重新走协作设计流程从头来。
      但局部编辑（"把第 N 段改成 X"）任何路径都支持。
```

**Plan and Execute 模式**：
- **Plan**（第一次交互）：收集信息 → 输出待办清单 → 等待用户确认
- **Execute**（第二次交互开始）：严格按待办执行 → 每步打卡 → 自动推进
...
**子流程**：
- [`templates/designs/video_design_guide.md`](./templates/designs/video_design_guide.md) —— design.md 步骤 0~11 子流程（被第二次交互**协作设计**和**自主设计**共用）
...
## 二、第一次交互：理解需求 + 路径选择 + 模式选择 + 制定待办（Plan 阶段）
...
### Plan and Execute + 状态机模式（强制）

> **核心原则：先制定待办，再严格执行。严禁边想边做、跳步、漏步。**
> **状态机约束：每个关键函数执行前会检查当前步骤，不满足前置条件直接报错阻止执行。**
...
| 步骤 | 含义 | 对应操作 |
|------|------|---------|
| `init` | 初始化完成 | 创建目录、生成 ID、确定模式 |
| `design` | design.md 已生成 | 调用 `writeDesignMd()` |
| `design_confirmed` | design.md 已确认 | 用户确认或 LLM 自检 |
| `project_json` | project.json 已生成 | 写入 project.json |
| `validated` | 校验通过 | 本地自检 + 云端预校验 |
| `packaged` | 已打包 | 打包 zip |
...
2. **状态机强制约束**：如果 LLM 尝试跳步（如没写 design.md 就直接写 project.json），函数会报错阻止
3. 完成当前步骤后，**自动进入下一步**，不需要用户催促
4. 如遇错误，修复后从当前步骤继续，不跳过
...
阶段三：设计确认
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
□ 4. [协作设计] 用户确认 design.md → 标记 designConfirmed
   [自主设计] LLM 自我确认 → 标记 designConfirmed
...
B. 自主设计（适合 demo / 试错 / 短视频 / 一次性预览）
   生成完整 design.md（不需你确认，LLM 自我确认）→ 直接打包上传
   优点：省掉用户等待确认的时间；缺点：用户无法在 design 阶段介入调整
```

**设计路径选择的严格规则**：
- 用户明示"快速 / 简单 / demo / 试一下 / 随便做个" 等关键字 → 仍要给两条路径选项确认，**不要悄悄替用户决定**
- 用户没回路径 → 默认 **A 协作设计**（保守、可控）
- 用户选 B 自主设计后：跳过第三次交互（用户确认环节），第二次生成完整 design.md 后由 LLM 自我确认，**自动进入第四次**
...
```
我帮你做这个视频，简单确认下：
1. 设计路径：A 协作设计（生成完整 design.md + 你确认） / B 自主设计（直接生成）
2. 视频内容：你想表达什么？（哪怕一句话也行，比如"介绍我们的产品优势"或"分享一个学习方法"）
3. 时长：5 / 15 / 30 / 60 秒
4. 目标受众：大众 / 专业人士 / 年轻人？

不回我就用默认：A 协作设计 + 15 秒 + 大众受众。
但"内容"不能默认——没有内容我没法创作。
```
...
我帮你做这个口播视频，简单确认下：
1. 设计路径：A 协作设计（生成完整 design.md + 你确认） / B 自主设计（直接生成）
2. 音频文件路径：`.mp3 / .wav / .m4a`（只认这三种扩展名，其他格式不算音频素材）
3. SRT 字幕文件路径：`.srt`（只认 .srt 格式，其他格式不算字幕素材）
...
**自主设计（路径 B）创作模式必问 2 项**（受众默认大众）：

| 字段 | 询问理由 | 默认值兜底（用户拒答时） |
|------|---------|-----------------------|
| **视频内容** | 决定文案、画面、组件要表达什么 | **无默认值——必须用户提供，哪怕一句话** |
| **视频时长** | 决定区域数、节奏密度 | **15 秒**（自主设计专属默认值） |

**口播模式必问 2 项**（不论协作设计还是自主设计，**绝不问时长和受众**）：
...
- 创作模式：首条已含路径 + 内容 + 时长 + 受众 → 不再追问；缺哪问哪
- 口播模式：首条已含路径 + 音频 + SRT 路径 → 不再追问；缺哪问哪（**绝不问时长、受众**）
- 用户在补充消息中明确说"按默认来 / 你看着办 / 随便"时，使用默认值兜底继续
- **主题非必问**（默认 black 沉浸黑），不要强制二选一
- 口播模式必须校验 `.mp3/.wav/.m4a + .srt` 文件存在
- 创作模式**默认配 BGM**（除非用户主动拒绝）
- **路径选择确定后必须把 `path` 字段写到 `project-state.json`**：`standard` = 协作设计，`quick` = 自主设计。后续脚本会读取该字段判断流程分支
...
- ❌ 强行追问主题 / 受众（创作模式才问受众；主题永远不问；自主设计连受众都不问；**口播模式绝不问时长和受众**）
...
- ❌ **自主设计跳过 batch 查 API、validate.js 自检、云端 precheck（这三条硬规则不放宽）**
- ❌ **用户已选自主设计后又跳过 design.md 直接写 project.json（自主设计仍需完整设计文档）**
...
**[协作设计]**：[`templates/designs/video_design_guide.md`](./templates/designs/video_design_guide.md) —— 详细规定 design.md 的"步骤 0 + 五阶段十一步 + 用户素材清单"产出格式。LLM 必须按子流程的 11 个步骤**逐步输出，不能跳步、不能一次性输出全部**。生成后进入第三次交互等用户确认。

**[自主设计]**：[`templates/designs/video_design_guide.md`](./templates/designs/video_design_guide.md) —— 与协作设计共用同一套完整设计文档。**逐步输出 11 个步骤，不能跳步**。生成后由 LLM 自我确认（调用 `state.markDesignConfirmed()`），**自动跳过第三次交互**直接进入第四次。
...
### 必查规则
- **协作设计**与**自主设计**：都查所有 references/ 规则（子流程会逐步引用），自主设计不放宽任何规则

### 严禁
- ❌ 跳过子流程任何步骤
- ❌ 一次性输出所有产出（协作设计与自主设计都必须按步骤逐步生成）
- ❌ 留空表格单元格 / 自检全部填"通过"
- ❌ 使用 `theme` 之外的全局主题机制（必须用 customStyle）
- ❌ 组件 ID 用英文单词（必须用 `{区域}-###` 格式）
- ❌ **自主设计跳过 design.md 直接写 project.json**（完整设计文档仍是必要的思考脚手架）
...
> ⚠️ **自主设计跳过本节**——design.md 生成后由 LLM 自我确认，自动进入第四次，不等用户确认。
> 本节只对协作设计生效。
...
- **自主设计下** LLM 在生成 design.md 并完成自检后**立即调用** `state.markDesignConfirmed()` 自我标记为已确认，无需用户介入（这是自主设计的核心简化：省掉用户等待确认的时间，设计质量不打折）
...
如果本次走的是**自主设计**，输出文案的**最开头**必须加这一段：

```
⚡ 自主设计产出 — 已跳过用户确认环节，设计质量与协作设计一致
   • 想完善设计：发"按协作设计重做"重新走一遍（不复用本次产物）
