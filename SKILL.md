...
- 理解用户视频主题、时长、受众等基本信息
- 选定**模式**（创作 / 口播）
- 选定**主题**（white / black 二选一）

### 必查规则
- [`mode-rules.md`](./references/mode-rules.md) §1（模式定义）、§3（字幕共生）、§4（创作模式默认 BGM）
- [`themes-catalog.md`](./references/themes-catalog.md)（主题二选一话术、自定义主题应对）
- [`principles.md`](./references/principles.md) §2（必问 vs 可默认 清单）

### ⚠️ 必问清单（用户首条消息缺失则必须追问，禁止直接开工）

用户首条消息只给了"做一个 XX 视频"这样的方向时，**严禁立即开始生成 design.md**。
必须先用 **AskUserQuestion 工具一次性收集** 以下 4 项必要信息（缺一不可）：

| 字段 | 询问理由 | 默认值兜底 |
|------|---------|-----------|
| **模式**（创作 / 口播） | 决定是否需要 SRT、是否生成字幕、节奏门槛 | 用户拒答则默认创作模式 |
| **视频时长** | 决定区域数、节奏密度、推荐区域数 | 用户拒答则默认 60 秒 |
| **目标受众** | 决定文案口吻、专业度、参考示例风格 | 用户拒答则默认"大众用户" |
| **主题**（white / black 二选一） | 决定全局配色 | 用户拒答则默认 white |

**口播模式额外必问**：mp3/wav/m4a 音频路径 + SRT 字幕路径（缺一不可）。

**询问方式**：使用 AskUserQuestion 工具，把上述 4 项做成 1-4 个并列问题一次性问完，**不要逐个追问**。

### 执行要点
- 用户首条消息已含上述 4 项必要信息时（明示或可合理推断），**不再追问**，直接进入第二次交互
- 用户首条消息**缺失任何一项**必要信息时，**必须先用 AskUserQuestion 一次性补齐**，不允许凭默认值直接开工
- 用户在补充信息中明确说"按默认来 / 你看着办 / 随便"时，再使用上表的默认值
- **主题二选一必问**（除非用户已主动选）；不允许伪造其他主题
...
1. **第一次交互必问 4 项基础信息**：模式 / 时长 / 受众 / 主题（缺失任一项必须用 AskUserQuestion 一次性收集，详见 [`principles.md`](./references/principles.md) §2）
2. **设计文档仅在本地**：不上传服务器
